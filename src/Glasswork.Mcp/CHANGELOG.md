...
- **`update_task` tool** (issue #136): enables agents to modify existing tasks. Accepts `task_id` plus up to 8 optional fields (`title`, `status`, `description`, `notes`, `priority`, `parent_task_id`, `ado_link`, `ado_title`). Only provided fields are written; omitted fields remain untouched on disk. Returns `{ task_id, updated_fields[] }` listing field names that actually changed (no-op fields excluded). Special handling: `notes` supports `notes_append: bool` to append with blank line separator (`existing.TrimEnd() + "\n\n" + new`) instead of replacing; empty/null `parent_task_id` clears the parent. Error cases: `not_found` (task doesn't exist), `invalid_status` (unknown status value), `invalid_parent` (parent task doesn't exist). Write registered with `SelfWriteCoordinator` to suppress App's external-change banner. Trace phase `write` instrumented under `GLASSWORK_MCP_TRACE=1`.
