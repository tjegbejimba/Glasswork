...
public sealed class ResourceMutationService
{
        ...
        JsonElement fields)
    {
        ...
        try
        {
            using (VaultScopedCoordinator.EnterExclusive(_vaultPath))
            {
                ...
                result = TransactUnsafe(mutationId, taskId, ifRevision, fields, notifications);
            }
        }
        catch
        {
            // A process crash can leave the journal between replacement and the
            // commit marker. Recover after the original lease is gone so the
            // recovery path can safely notify in-process index subscribers.
            try
            {
                using (VaultScopedCoordinator.EnterExclusive(_vaultPath))
                    notifications.UnionWith(RecoverUnsafe());
            ...
            {
                // Preserve the original operation failure. A later managed
                // access will retry recovery and surface a durable error if needed.
            }
    ...
    private ResourceMutationOutcome TransactUnsafe(
        string? mutationId,
        string? taskId,
        string? ifRevision,
        JsonElement fields,
        ISet<string> notifications)
    {
        ...
        // Re-read after staging and immediately before journaling/replacement.
        // This closes the normal unmanaged-edit window; the final revision is
        // never inferred from the first read.
        ...
        // Compare canonical task meaning rather than serialized bytes. A
        // no-op must preserve hand formatting, line endings, and comments.
    ...
    private ResourceMutationOutcome Record(
        State state,
        string id,
        string hash,
    ...
    private string? ApplyFields(GlassworkTask task, JsonElement fields)
    {
        ...
        foreach (var property in fields.EnumerateObject())
        {
            ...
            {
                case "title":
                    task.Title = ReadString(property.Value, property.Name) ?? string.Empty;
                    break;
                case "status":
                ...
                case "blocked_from_status":
                    ...
                    break;
                case "priority":
                    task.Priority = ReadString(property.Value, property.Name) ?? string.Empty;
                    break;
                case "type":
                    task.Type = GlassworkTask.Types.Normalize(ReadString(property.Value, property.Name));
                    break;
                case "parent_task_id":
                    task.Parent = ReadString(property.Value, property.Name);
                    break;
                case "description":
                    task.Description = ReadString(property.Value, property.Name) ?? string.Empty;
                    break;
                case "notes":
                    if (property.Value.ValueKind == JsonValueKind.Object
                        && property.Value.TryGetProperty("append", out var append))
                    {
                    ...
                    {
                        task.Notes = ReadString(property.Value, property.Name) ?? string.Empty;
                    }
                    break;
                case "due_date":
                    task.Due = ReadDate(property.Value, property.Name);
                    break;
                case "scheduled":
                    task.MyDay = ReadDate(property.Value, property.Name);
                    break;
                default:
                    return $"Unsupported task field '{property.Name}'.";
            }
        }
    ...
    private ResourceMutationTaskSnapshot Snapshot(GlassworkTask task, string revision) =>
        new(
            task.Id,
            task.Title,
            task.Status == GlassworkTask.Statuses.InProgress ? "doing" : task.Status,
            task.Priority,
            task.Type,
            task.Created,
            task.CompletedAt,
            ...
            task.BlockedReason,
            task.BlockedAt,
            task.BlockedFromStatus is null
                ? null
                : task.BlockedFromStatus == GlassworkTask.Statuses.InProgress
                    ? "doing"
    ...
    {
        public Dictionary<string, RecordedOutcome> Outcomes { get; set; } =
            new(StringComparer.Ordinal);
    }

    private sealed record RecordedOutcome(
        string RequestHash,
    ...
    private void Prune(State state)
    {
        ...
        foreach (var key in state.Outcomes
                     .Where(x => x.Value.CreatedAt <= cutoff)
                     .Select(x => x.Key)
                     .ToList())
    ...
    private IReadOnlyList<string> RecoverUnsafe()
    {
        ...
        try
        {
            entry = JsonSerializer.Deserialize<JournalEntry>(File.ReadAllText(JournalPath))
